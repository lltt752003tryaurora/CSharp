    ...
    public class CLogger
    {
        ...
        public static string CreateLogMessage(DateTime startDate, string functionName, string code, string data)
        {
            try
            {
                ...
                string serverIp = "";
                return CLogger.CreateLogMessage(serverId, serviceName, functionName, "", code, t.TotalMilliseconds, serverIp, "", "", data);
            }
            catch (Exception ex)
            {
                CLogger.WriteInfo("====================================================Create log data exception:" + ex.Message);
                return "NULL";
            }
        }
        /// <summary>
        ...
        public static void WriteLogSuccess(string message)
        {
            try
            {
                int enable = ConfigHelper.GetConfig("EnableLogSuccess", 0);
                if (enable == 1)
        ...
        public static void WriteLogError(string message)
        {
            try
            {
                int enable = ConfigHelper.GetConfig("EnableLogError", 0);
                if (enable == 1)
        ...
        public static void WriteLogException(string message)
        {
            try
            {
                int enable = ConfigHelper.GetConfig("EnableLogException", 0);
                if (enable == 1)
                {
        ...
        public static void WriteInfo(string message)
        {
            try
            {
                int enable = ConfigHelper.GetConfig("EnableLogInfo", 0);
                if (enable == 1)
